...
private:
                    ...
                    if (_this->useFrequencyManager) {
                        ...
                        if (ImGui::IsItemHovered()) {
                            ImGui::SetTooltip("FREQUENCY MANAGER + FFT MODE\n"
                                              ...
                                              "\n"
                                              "Perfect for covering wide frequency ranges quickly yet thoroughly",
                                              _this->interval / 1e3);
                        }
                    }
                    else {
    ...
    bool isFrequencyBlacklisted(double frequency) const {
        const int CMD_IS_FREQ_BLACKLISTED = 3;
        bool result = false;
        if (core::modComManager.interfaceExists("frequency_manager")) {
            if (!core::modComManager.callInterface("frequency_manager", CMD_IS_FREQ_BLACKLISTED,
                                                   const_cast<double*>(&frequency), &result)) {
                result = false;
            }
        }
        return result;
